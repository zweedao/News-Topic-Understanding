...
def load_data(filename):
    ...
    data.columns = ['id','text_t']
    data = data.reset_index(drop=True)
    return data

def load_labels(filename):
    labels = pd.read_csv(filename)
    labels.columns = ['id','newsgroup_n']
    labels = labels.reset_index(drop=True)
    labels = np.array(labels['newsgroup_n'])
    return labels
...
def clean_df(df):
    #lowercase the text
    df['text_t'] = df['text_t'].str.lower()

    #strip punctuation
    df['text_t'] = df['text_t'].str.replace('[^a-zA-Z ]', " ")

    #tokenize
    df['text_t'] = df['text_t'].apply(get_words)

    return df

def get_bow_dict(df):
    wordfreq = {}
    for sentence in df['text_t']:
...
def bag_of_words(sentences, word_indices):
    sentence_vectors = []
    for sentence in sentences:
        current_vector = [0 for _ in range(len(word_indices))]
        for token in sentence:
            if token in word_indices:
                current_vector[word_indices[token]] = 1
        sentence_vectors.append(current_vector)
    return np.array(sentence_vectors).astype(int)
